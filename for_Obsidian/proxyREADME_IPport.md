...
==Система шифрования, Fernet в библиотеке cryptography, работает следующим образом:==

1. Берет исходные данные (например, JSON от клиента).
2. Шифрует их с помощью алгоритма AES.
3. Добавляет цифровую подпись (HMAC) для проверки целостности.
4. Кодирует весь результат (шифротекст + подпись + метаданные) с помощью Base64 (URL-safe варианта). Base64 как раз использует символы латинского алфавита (A-Z, a-z), цифры (0-9), а также символы - и _, и иногда символ = для выравнивания.

Итого, все что в виде HEX-строки на сервере — это Base64-представление зашифрованных данных. Это ожидаемое поведение  системы. Сервер должен взять эту Base64-строку, декодировать ее обратно в байты, а затем расшифровать эти байты с помощью ключа, чтобы получить исходное сообщение от клиента.
